...
The data set should minimally contain the following 5 variables: (1)
...
**Note:** The `occasion` and `group` variables are optional when using the `bspam` prep() functions. If the data do not contain an occasion variable, `bspam` internally assigns all observations to a default occasion (`occasion = 1`). Similarly, if the data do not contain a group variable, all observations are assigned to a default group (`group = 1`). Therefore, users do not need to manually create these columns when their data contain only one occasion or when group information is not available.
...
A case is identified by the combination of the person ID and testing occasion, separated by an underscore (for example, 2033_Fall). The group variable is not part of the case identifier. It is treated as descriptive information associated with the person-occasion case and is retained in the scoring output.

If no occasion variable is supplied, bspam internally creates a single default occasion with the value 1. Thus, all observations for a person belong to the same occasion.
