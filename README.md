...
accuracy is modeled by a binomial count latent variable model. For
example, the use of this modeling technique allows model-based
...
frame, where each row is data for a unique case, namely, a specific task
from a specific student in a specific testing occasion. The data set
should minimally contain the following 7 variables: (1) individual ID,
(2) group ID, (3) testing occasion ID, (4) task ID, (5) the maximum
number of attempts in the task, (6) the number of successes in the task,
and (7) the time took to complete the task.
...
basic usage of key functions in the package, an example ORF assessment
...
success) or not. The number of successes in the task is the number of
words correctly read in the passage.
