...
getBootstrapSE <- function (object, person.data, case=NA, perfect.cases, zero.cases, est="map", kappa=1, bootstrap=100, external=NULL) {
  ...
  if (est == "mle") {
    ...
    # get theta.mle
    WCPM_mle <- MCEM %>% run.scoring(person.data, task.data=task.data, cases=case, perfect.cases, zero.cases, est='mle', lo = -4, hi = 4, q = 100, kappa = 1, external=external, type="orf")
